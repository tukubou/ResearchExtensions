(async () => {
  ...
  if (href.includes('https://www.amazon.co.jp/s/')) {
    // 検索結果画面
    localStorage.clear();
    const divInfoBar = /** @type {HTMLElement} */ (document.querySelector(
        '[id=s-result-info-bar-content]'
    ));
  ...
  **/
  function getHtmlData(waitTime, bTag) {
    let index = 0;
    const liResult = /** @type {HTMLElement} */ (document.querySelectorAll(
        'li[id*=result_]'
    ));
    const intervalId = setInterval(() => {
      ...
      if (liResult[index].getElementsByTagName('h5').length === 0) {
        ...
        const spacingBase = liResult[index].getElementsByClassName(
            'a-spacing-base'
        );
  ...
  **/
  function getSelerData(index, rankingLimit, gasUrl) {
    const aBrand = /** @type {HTMLElement} */ (document.querySelector(
        '[id=bylineInfo]'
    ));
    const spanPrice = /** @type {HTMLElement} */ (document.querySelector(
        '[id=priceblock_ourprice]'
    ));
    const divEnrichContainer = /** @type {HTMLElement} */ (document.querySelector(
        '[class=enrich-container]'
    ));
    ...
    const ranking = spanEnrichResult[1].textContent
        .replace(/位/g, '')
        .replace(/,/g, '');
    const image = /** @type {HTMLElement} */ (document.querySelector(
        '[id=landingImage]'
    ));
    ...
    for (const [key] of map) {
      if (key === 'total-review-count') {
        const countRaw = /** @type {HTMLElement} */ (document.querySelector(
            `h2[data-hook='${key}']`
        ));
        map.set(
            key,
          !countRaw
            ? 0
            : Number(
                countRaw.textContent.replace(/件のカスタマーレビュー/g, '')
            )
        );
      } else {
        const countRaw = /** @type {HTMLElement} */ (document.querySelector(
            `a[class*='a-size-base a-link-normal ${key} histogram-review-count']`
        ));
        map.set(key, !countRaw ? 0 : Number(countRaw.textContent.slice(0, -1)));
      }
    }
    const reviewGood = Math.round(
        ((map.get('4star') + map.get('5star')) * map.get('total-review-count')) /
        100
    );
    const reviewBad = Math.round(
        ((map.get('1star') + map.get('2star') + map.get('3star')) *
        map.get('total-review-count')) /
        100
    );
