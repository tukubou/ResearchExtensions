  ...
  function getSelerData(index, rankingLimit, gasUrl) {
    ...
    for (const [key] of map) {
      if (key === 'total-review-count') {
        const countRaw = /** @type {HTMLElement} */ (document.querySelector(`h2[data-hook='${key}']`));
        map.set(key, (!countRaw) ? 0 : Number(countRaw.textContent.replace(/件のカスタマーレビュー/g, '')));
      } else {
        const countRaw = /** @type {HTMLElement} */ (document.querySelector(`a[class*='a-size-base a-link-normal ${key} histogram-review-count']`));
